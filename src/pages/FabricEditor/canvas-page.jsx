...
class Page extends Component {
  ...
  handleAfterMouseClickOnCanvas(e) {
    ...
    if (!this.canvasRef?.getActiveObject()) return;
    const editableProps = this.extractEditableProps(e);
    if (!this.props.showStyleEditor) {
      // calculate new offsets for <PlaceElement>
  ...
  async initCanvas() {
    ...
    this.canvasRef = _canvas;
    // keep track of elements rendered initially
    this.setState({ elementsRendered: elements.map((e) => e?.id) });
    let ElemtoActivate = null;
    try {
      ElemtoActivate = await this.generatePageContent(elements, _canvas);
    } catch (error) {
      console.log(error);
    } finally {
      if (ElemtoActivate) {
        this.handlePageItemClick(null, ElemtoActivate, _canvas);
        this.props.ontemplateLoaded(ElemtoActivate, _canvas);
      }
    }
  ...
  cloneObject = (eventData, transform, activeElem, canvasRef) => {
    ...
    if (activeObject.customType === "SpeechBubble") {
      if (activeObject.isLabel === true) {
        ...
        this.addSpeechBubble(bubbleElementSchema, true, canvas);
      } else {
      ...
      this.addQuadratic(quadraticElementSchema, canvasRef);
      ...
      if (activeObject.subType === "svg") {
        const objs = [];
        activeObject._objects.forEach((item) => {
          objs.push(item);
        });
        const newSvg = new fabric.Group(objs, {
          id: getNewID(),
          name: activeObject.name + " clone",
          subType: activeObject.subType,
          selectable: activeObject.selectable,
          sendtoback: activeObject.sendtoback,
        });
        this.cloneElement(newSvg, canvas);
      } else {
  ...
  cloneElement = (activeObject, canvas) => {
    activeObject.clone((cloned) => {
      ...
      if (cloned.type === "i-text") {
        cloned.changedName = true;
        cloned.customName = activeObject.name + " 2";
      }
      ...
      setTimeout(() => {
        ...
        canvas.requestRenderAll();
        if (
          ["rect", "circle", "triangle", "i-text"].includes(activeObject.type)
        )
          if (activeObject.patternActive) {
            cloned.URL = activeObject.URL;
            addPattern(activeObject.URL, canvas, () => {
              this.handlePageItemClick(null, cloned);
              canvas.requestRenderAll();
            });
          }
  ...
  keydownListener = (event) => {
    ...
    if (key === "Delete" || key === "Backspace") {
      ...
      if (this.props.isCanvasActive) this.deleteObject();
    }
    //ctrl+z key press
  ...
  addLeftArrow(activeObject, angleType) {
    const leftTriangleElement = this.getTriangleElement("Left", activeObject);
    ...
    return leftArrowGroup;
  }

  addRightArrow(activeObject, angleType) {
    ...
    return rightArrowGroup;
  }

  addDoubleArrow(activeObject, angleType) {
  ...
  createArrowStyle(activeObject) {
    ...
    switch (angleType) {
      case ARROW_HEAD.LEFT_ARROW:
        arrowElement = this.addLeftArrow(activeObject, angleType);
        break;
      case ARROW_HEAD.RIGHT_ARROW:
        arrowElement = this.addRightArrow();
        break;
      case ARROW_HEAD.DOUBLE_SIDED:
        arrowElement = this.addDoubleArrow(activeObject, angleType);
        break;
      default:
  ...
  componentDidUpdate(prevProps) {
    ...
    if (this.props.activeElementProps !== prevProps.activeElementProps) {
      ...
      if (activeObject instanceof fabric.IText) {
        ...
        activeObject.getObjects().forEach((obj, index) => {
          if (obj.strokeWidth && obj.stroke) {
            obj.set({ stroke: strokeColors[index] });
          }
        });
      }
      this.canvasRef.renderAll();
  ...
  handlePageItemClick(event, item, _canvas) {
    ...
    this.props.onElemSelect(true, newActiveElementProps, _canvas);
  }
  ...
  extractEditableProps(event, item) {
    ...
    let strokeColors = this.generatestrokeColorInfo(_target);
    editableProps.colors = colors;
    editableProps.strokeColors = strokeColors;
  ...
  /**
   * extract strokecolor inforation from selected object and send to parent
   * @param {HTML Event} event canvas click event
   */
  generatestrokeColorInfo(target) {
    const colors = [];
    if (target.hasOwnProperty("_objects")) {
      // target is a "group"
      target._objects.forEach((object) => {
        // check if color is already picked
        if (object.stroke || object.strokeWidth) colors.push(object.stroke);
      ...
      // svg path (instance of klass)
      if (typeof target.stroke !== "object") {
        if (target.stroke) colors.push(target.stroke);
        return colors;
      } else {
        return colors;
      }
  ...
  checkElementPosition(elem, _canvas) {
    if (
      elem.type === "line" ||
      elem.customType === "Quadratic" ||
      (!elem.left && !elem.top && elem.type !== "line") ||
      elem.customType === "SpeechBubble"
    ) {
      alignElementToCenter(_canvas, _canvas.width, _canvas.height, elem);
      this.SaveAndShiftElementPosition(elem);
    }
  }

  handleElementInit(elem, _canvas) {
    this.handlePageItemClick(null, elem, _canvas);
    _canvas.setActiveObject(elem);
    this.checkElementPosition(elem, _canvas);
  }

  async addTextElementToPage(elem, _canvas) {
    const textElement = await this._core.getIText(elem.value, elem);
    _canvas.add(textElement);
    if (elem.selectable) {
      textElement.on("mousedown", this.handlePageItemClick);
    }
    this.handleElementInit(textElement, _canvas);
  }

  async addSvgElementToPage(elem, _canvas) {
    const svgElement = await this._core.getSVGFromURL(elem.url, elem);
    console.log(svgElement);
    // _canvas.add(svgElement);
    if (elem.selectable) {
      svgElement.on("mousedown", this.handlePageItemClick);
    }
    this.handleElementInit(svgElement, _canvas);
  }

  addTriangleElementToPage(elem, _canvas) {
    const triangleElement = this._core.getTriangle(elem);
    _canvas.add(triangleElement);
    if (elem.selectable) {
      triangleElement.on("mousedown", this.handlePageItemClick);
    }
    this.handleElementInit(triangleElement, _canvas);
  }

  addCirlceElementToPage(elem, _canvas) {
    const circleElement = this._core.getCircle(elem);
    _canvas.add(circleElement);
    if (elem.selectable) {
      circleElement.on("mousedown", this.handlePageItemClick);
    }
    this.handleElementInit(circleElement, _canvas);
  }

  addLineElementToPage(elem, _canvas) {
    const lineElement = this._core.getLine(elem);
    _canvas.add(lineElement);
    if (elem.selectable) {
      lineElement.on("mousedown", this.handlePageItemClick);
    }
    this.handleElementInit(lineElement, _canvas);
  }

  addRectangleElementToPage(elem, _canvas) {
    const rectElement = this._core.getRect(elem);
    _canvas.add(rectElement);
    if (elem.selectable) {
      rectElement.on("mousedown", this.handlePageItemClick);
    }
    this.handleElementInit(rectElement, _canvas);
  }

  async addImageElementToPage(elem, _canvas) {
    const { containerRect, newProps } = await this._core.addImgFromURL(
      elem.url,
      elem
    );
    if (elem.selectable) {
      containerRect.on("mousedown", this.handlePageItemClick);
    }
    const editableProps = this.extractEditableProps(null, containerRect);
    let newActiveElementProps = {
      ...editableProps,
      ...newProps,
      selectedElementName: containerRect.name,
      id: containerRect.id,
    };
    this.props.onElemSelect(true, newActiveElementProps, _canvas);
  }

  async addImgAsPatternFromURL(elem, _canvas) {
    ...
    this.handlePageItemClick(null, patternImage, _canvas);
  }

  addQuadratic(elem, _canvas) {
    ...
    _canvas.add(group);
    this.handleElementInit(group, _canvas);
  }
  ...
  async addSpeechBubble(elem, isLabel, _canvas) {
    //create polygon
    ...
    _canvas.offHistory();
    _canvas.add(SpeechBubble);
    this.handleElementInit(SpeechBubble, _canvas);
    _canvas.onHistory();
  }
  ...
  handleSvgStr = (elem) => {
    fabric.loadSVGFromString(elem.svgStr, (objects, options) => {
      ...
      // loadedObject.selectable = elem.selectable;
      // loadedObject.sendtoback = elem.sendtoback;
      console.log(loadedObject);
      this.canvasRef.add(loadedObject);
      this.handlePageItemClick(null, loadedObject);
      this.canvasRef.setActiveObject(loadedObject);

      // const svgElement = await this._core.addSVGFromURL(elem.url, elem);
      if (elem.selectable) {
        loadedObject.on("mousedown", this.handlePageItemClick);
      }
      this.handleElementInit(loadedObject, this.canvasRef);
    });
  };

  async generatePageContent(elements, _canvas) {
    let canvas = _canvas ? _canvas : this.props._canvas;
    for (const elem of elements) {
      switch (elem.type) {
        case "i-text":
          await this.addTextElementToPage(elem, canvas);
          break;
        case "Svg":
          await this.addSvgElementToPage(elem, canvas);
          break;
        case "svg":
          this.handleSvgStr(elem, canvas);
          break;
        case "triangle":
          this.addTriangleElementToPage(elem, canvas);
          break;
        case "circle":
          this.addCirlceElementToPage(elem, canvas);
          break;
        case "line":
          this.addLineElementToPage(elem, canvas);
          break;
        case "rect":
          this.addRectangleElementToPage(elem, canvas);
          break;
        case "Image":
          await this.addImgAsPatternFromURL(elem, canvas);
          break;
        case "Pattern":
          await this.addImgAsPatternFromURL(elem, canvas);
          break;
        case "Quadratic":
          this.addQuadratic(elem, canvas);
          break;
        case "speech_bubble":
          this.addSpeechBubble(elem, false, canvas);
          break;
        case "speech_label":
          this.addSpeechBubble(elem, true, canvas);
          break;
        default:
  ...
  render() {
    ...
    return (
      <div className="page" style={style} ref={this.containerRef}>
        <canvas
          id={`canvas-${pageId}`}
